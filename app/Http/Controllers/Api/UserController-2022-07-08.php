    ...
    public function distributorstore(Request $request): JsonResponse
    {
       ...
       public function distributorlist(): JsonResponse
       {
        ...
        //$activity = $this->userRepository->distributorlist();
		   $activity = DB::select("select distributors.*,users.id as distributor_id from distributors join users on distributors.bussiness_name = users.name where distributors.bussiness_name in (select distinct distributor_name from retailer_list_of_occ where ase like '%$ase%')");
